...
import { RouteLocationNormalized, RouteRecordRaw, useRoute } from "vue-router";
    ...
    setup() {
        ...
        const toLastView = (visitedViews: TagView[], view: TagView) => {
            ...
            if (latestView !== undefined && latestView.fullPath !== undefined) {
                ...
                if (view.name === "Dashboard") {
                    // to reload home page
                    router
                        .push({ path: "/redirect" + view.fullPath })
                        .catch(err => {
                            console.warn(err);
                        });
                } else {
        ...
        const state = reactive({
            ...
            closeAllTags: (view: TagView) => {
                ...
                if (
                    state.affixTags.some(tag => tag.path === currentRoute.path)
                ) {
                    return;
                }
                toLastView(store.state.tagViews.visitedViews, view);
